...
### Сценарий 2: Генерация конфига из существующих репозиториев (инвентаризация)

Если у вас уже есть директория с Git-репозиториями, команда `generate` рекурсивно просканирует её, найдет все репозитории (по наличию `.git`) и сформирует первичный конфиг из реально найденных рабочих копий: имя, URL `origin` (если доступен) и абсолютный путь. Это снимок вашей текущей структуры, а не чистый шаблон со всеми настройками:

```bash
# 1. Инвентаризируем текущую директорию: все вложенные git-репозитории попадут в конфиг
go run ./cmd/go-repo-orchestrator generate --config ./my-repo.gbc.yaml

# 2. Отредактируйте my-repo.gbc.yaml. Команда заполняет только repos[].name/url/path —
#    секцию jira, browser и правила branch.keep / branch.jira / branch.autocheck
#    нужно добавить вручную под свой workflow.

# 3. Запускаем оркестратор
go run ./cmd/go-repo-orchestrator --config ./my-repo.gbc.yaml
```

Если нужен образец конфигурации со всеми секциями и демонстрационными значениями — возьмите статический файл `config.example.yaml` из сценария 1 и адаптируйте его под себя.
...
Статический демонстрационный пример со всеми секциями находится в файле [`config.example.yaml`](config.example.yaml) — это готовый файл в репозитории, его можно скачать и адаптировать (см. Быстрый старт, сценарий 1). Не путайте его с командой `generate`: она выполняет инвентаризацию текущей директории и включает в конфиг только найденные Git-репозитории (`repos[].name/url/path`), а остальные секции (`jira`, `browser`, `branch.*`-правила) добавляются вручную. Обязательным требованием является передача файла конфигурации через флаг `--config`.
...
Команда `generate` сканирует текущую директорию и записывает в конфиг только найденные Git-репозитории (имя, URL `origin`, путь). Она не может угадать:
...
### Переменные окружения в значениях конфига

Во всех строковых значениях YAML поддержаны плейсхолдеры в синтаксисе docker-compose:

```yaml
jira:
  - group: "MYPROJ"
    url: "https://jira.corp.local"
    token: ${MYPROJ_JIRA_TOKEN}              # обязательная переменная
    login:
      username: ${MYPROJ_JIRA_USER:-}        # пустой дефолт — поле необязательное
    ssl:
      client_cert: ${MYPROJ_CERT:-/etc/ssl/corp/jira-client.pem}
browser:
  cdp_url: ${GRO_CDP_URL:-http://127.0.0.1:9222}
```

Правила:

- `${VAR}` — подставляется значение переменной; если переменная не задана, старт завершается понятной ошибкой (имя переменной + путь ключа), а не тихой подстановкой пустой строки.
- `${VAR:-default}` — значение по умолчанию, если переменная не задана или пустая; `${VAR:-}` легализует необязательное поле.
- Имена переменных произвольные — они задаются прямо в YAML, утилита их не зашивает.
- Литеральный `$` экранируется как `$$` — актуально для regex-правил `branch.keep`/`branch.jira`, где одиночный `$` (якорь конца строки) и так не трогается.
- Поддерживается только подстановка и дефолт — это не шаблонизатор с условной логикой.

Такой конфиг можно коммитить в репозиторий команды: структура, пути и regex-правила общие, а токены, пароли и пути к сертификатам каждый участник держит в своём окружении.
...
Команда `doctor` позволяет изолированно отладить интеграции и загрузку репозиториев без запуска TUI:

#### Диагностика Jira: `doctor jira`

Если статусы задач показываются как `auth [A]` и непонятно почему — выполните один диагностический запрос с полным трейсом:

```bash
go-repo-orchestrator doctor jira PROJ-123 --config config.yaml
```

Команда определяет jira-группу по ключу задачи (named-group mapping из `repos[].branch.jira`; если ключ не замаплен — проверяет все группы), выполняет ровно один запрос **тем же транспортным кодом, что и TUI**, и печатает: группу, base URL, транспорт, тип авторизации (только факт, без секретов), итоговый URL, HTTP-статус, content-type, final URL, размер и санитизированный фрагмент тела (до 512 байт), вердикт классификатора auth (401 / 403 / login-redirect / HTML-ответ), а для browser-групп — результат CDP preflight, режим подключения и выбор контекста (сколько их, у скольких есть куки для домена, какой будет использован), включая факт и причину browser→HTTP fallback.

При недоступном браузере/CDP команда завершается ненулевым кодом; вывод можно перенаправить в файл и приложить к багрепорту.

#### Диагностика репозиториев: `doctor repo`

Если репозиторий в интерфейсе показывает «Ошибка доступа к репозиторию» или «Веток: 0» — выполните диагностику загрузки конкретного репозитория:

```bash
go-repo-orchestrator doctor repo group/repo --config config.yaml
```

Команда печатает: source-тип (`path` / `url` / `opensource`), URL (с маскированием учетных данных), абсолютный и относительный путь к каталогу репозитория, состояние каталога и Git (валидность HEAD, текущая ветка, количество локальных и remote-веток, upstream, незакоммиченные изменения), предупреждение об `autoswitch` (применение `reset --hard` и `clean -fd`), пробу синхронизации `fetch --prune origin` с полной ошибкой и понятный вердикт: «репозиторий загрузится» / «не тот каталог» / «клон-обрывок без коммитов» / «каталог не существует».
