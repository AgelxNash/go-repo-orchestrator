...
Падение advisory-проверки из-за внешнего сайта, rate limit, ограничения бота или нерелевантного для PR содержимого не является регрессией приложения. Владелец может merge корректный PR при успешном `ci / go-checks` и осознанном review diff. При игнорировании advisory-статуса (например, ложного срабатывания CodeQL или недоступности внешнего сервиса) рекомендуется зафиксировать краткое обоснование в комментарии к PR.
